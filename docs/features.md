...
- **Display can't drift from billing** — the public menu leads with a compact **"-%20" discount badge**, then the struck-through base price beside the live campaign price. Drawn in `currentColor` only, so each of the six templates renders it in its own palette with zero per-template work. The base price itself is untouched and the order path re-derives every billed line server-side: what the guest sees and what the receipt charges come from the same engine. Fractional campaign prices are shown **to the kuruş** (₺110,50 — never rounded to ₺111), because a shown price that differs from the billed price is a complaint waiting at the register
- **The owner sees it too** — item rows in the menu editor carry a live "-%20 · ₺104" chip sourced from the public menu endpoint (the very engine that prices guests), so the editor can never disagree with what the guest is being shown
